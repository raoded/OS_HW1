	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	...
	if(current->policy_enabled && current->policy_current_level < FORK_LEVEL){
		struct forbidden_activity_info activity = make_forbid(FORK_LEVEL, current->policy_current_level, jiffies);
	...
	*p = *current;
	...
	//hw1 start
	p->forbidden_queue = NULL;
	p->policy_enabled = 0;
	p->policy_current_level = 2;
	p->forbidden_max_size = 0;
	p->forbidden_next_index = 0
	//hwq end
