...
static void release_task(struct task_struct * p)
{
	...
	//hw1 start
	sys_disable_policy(p->pid, 234123);	//disable the policy if needed
	//hw1 end
...
asmlinkage long sys_wait4(pid_t pid,unsigned int * stat_addr, int options, struct rusage * ru)
{
	...
	if(current->policy_enabled && current->policy_current_level < WAIT_LEVEL){
		struct forbidden_activity_info activity = make_forbid(WAIT_LEVEL, current->policy_current_level, jiffies);
